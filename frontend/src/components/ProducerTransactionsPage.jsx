...
export default function ProducerTransactionsPage({ auctions }) {
  const { t, dir, locale } = useTranslation();
  const localeTag = locale === 'ar' ? 'ar-DZ' : locale === 'en' ? 'en-US' : 'fr-DZ';

  const rows = useMemo(() => {
    return auctions
      .filter(a => a.myBidId && a.status === 'closed')
      .map(a => {
        const bid = (a.bids || []).find(b => b.id === a.myBidId);
        const isWon = a.acceptedBidId === a.myBidId;
        const line = isWon ? resolveAcceptedLine(bid, a.acceptedLineId) : (bid?.lines?.[0] ?? null);
        return {
          id: a.id,
  ...
  const handleExport = () => {
    downloadCsv('sougra-transactions.csv',
      ...
      rows.map(r => ({
        ...
        status: r.won ? t('transactionsStatusWon') : t('transactionsStatusLost'),
      }))
    );
  };
  ...
  return (
    <div className="dash-page-scroll" style={{ flex: 1, padding: '32px 40px', overflowY: 'auto', textAlign: dir === 'rtl' ? 'right' : 'left' }}>
      ...
      <p className="no-print" style={{ color: 'var(--text-muted)', marginBottom: 24, fontSize: '0.95rem' }}>{t('transactionsPageDescProducer')}</p>
                ...
                <tr key={r.id} style={{ borderBottom: '1px solid var(--border)' }}>
                  ...
                  <td style={{ padding: '14px 18px' }}>
                    <span style={{
                      ...
                      background: r.won ? 'rgba(16,185,129,0.14)' : 'rgba(239,68,68,0.12)',
                      color: r.won ? 'var(--primary)' : '#ef4444',
                    }}>
                      {r.won ? t('transactionsStatusWon') : t('transactionsStatusLost')}
                    </span>
                  </td>
                </tr>
              ))}
